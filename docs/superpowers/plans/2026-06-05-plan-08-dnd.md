...
**Status:** 🚧 **ready to execute** (detailed to TDD on 2026-06-05; not yet started).
Expected gate at completion: full front-end suite green (currently **409** tests / 79 files at
plan-07) plus the new DnD tests; `cargo test -p handshaker` green with the new
`collection_move_item_across` tests; `pnpm lint` shows only the **15 pre-existing** legacy
errors (`features/collections/**` ×14 + `ipc/client.ts` ×1) — **zero new** under
`features/{catalog,workflow}` (legacy 15 are removed in plan-09, which also keeps `pnpm build`
`tsc`-blocked until then).
