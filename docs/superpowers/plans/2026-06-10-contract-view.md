...
### Task 18: подсветка полей вне контракта (диагностика)

Бэкенд отвергает неизвестные поля на Send: `invoke/mod.rs` десериализует тело через
`DynamicMessage::deserialize` с дефолтом prost-reflect `deny_unknown_fields = true` —
сейчас пользователь узнаёт об опечатке в ключе только из ошибки `EncodeRequest`.
Best practice (VS Code JSON schema): волнистая подсветка на ключе + hover-сообщение.
Severity — **Error** (у нас Send гарантированно упадёт). Только ключи (значения могут
быть `{{var}}`-плейсхолдерами); map-значения освобождены; в поддерево неизвестного поля
не спускаемся; при непарсибельном тексте маркеры не моргают (null → caller хранит
прежние, как VS Code); не под тогглом bodyHints (диагностика, не подсказка);
request-only.

**Files:**
- Create: `src/features/bodyview/validate.ts`, `src/features/bodyview/validate.test.ts`
- Modify: `src/features/bodyview/parse.ts` (`repairTrailingCommas` переезжает из ghost.ts,
  экспорт), `src/features/bodyview/ghost.ts` (импорт), `src/features/bodyview/hints.ts`
  (экспорт `pathTo`), `src/features/bodyview/BodyView.tsx` (склейка `setModelMarkers`)

- [x] **Step 1: Failing tests** — `computeUnknownFieldMarkers(text, schema)`:
  - неизвестный top-level ключ → один маркер, диапазон = `"bogus"` (включая кавычки),
    message `"bogus" is not a field of t.Req`;
  - все ключи известны → `[]`;
  - вложенный неизвестный ключ называет вложенное сообщение (`g.Ts`);
  - map-значения освобождены (произвольные ключи) → `[]`;
  - в поддерево неизвестного поля не спускаемся (один маркер, не каскад);
  - непарсибельный текст → `null`; висящая запятая ремонтируется (маркер есть).
- [x] **Step 2: Run** — FAIL (модуля нет). ✅
- [x] **Step 3: Implement** — чистое ядро: `parseWithSpans` (+ремонт) → walk по
  `tree.order`, `descendSchema(pathTo(...))`, неизвестный ключ → диапазон ключа
  обратным сканом от начала спана значения (фолбэк — спан значения при эскейпах).
  Склейка в `applyGhost` (та же каденция; игнорирует bodyHints; `markers === null` →
  оставить прежние; схема null → очистить); очистка маркеров при анмаунте.
  `repairTrailingCommas` переехал в `parse.ts` (общий для ghost+validate).
- [x] **Step 4: Run** — validate-тесты + полный гейт. ✅ 6/6 · lint clean · 752/752.
- [x] **Step 5: Commit** — ✅ `be64d37`.
