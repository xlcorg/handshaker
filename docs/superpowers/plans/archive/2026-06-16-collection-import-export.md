...
**Status:** 🎉 DONE 2026-06-17 — all 12 tasks implemented, final review APPROVED, rebased onto local `main` + ff-merged. Subagent-driven (impl per task + review on Task 4 merge logic + final whole-feature review). Extra over plan: env-list refresh also wired into `SavedAuthEditor` (review nit). On rebase onto `main` (which had independently added `tauri-plugin-dialog`), resolved mod-list / capability / import conflicts and de-duplicated the dialog dep+plugin. Post-rebase gate green: `cargo test --workspace` · `pnpm test` (975) · `pnpm build` · bindings no-drift. Remaining: live WebView2 pass against real save/open dialogs. Spec: [../../specs/archive/2026-06-16-collection-import-export-design.md](../../specs/archive/2026-06-16-collection-import-export-design.md).
