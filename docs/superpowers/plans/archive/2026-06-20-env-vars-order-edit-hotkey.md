...
**Status banner:** 🎉 DONE (live-verified в WebView2 2026-06-20; ff в `main`). Ветка `claude/peaceful-gauss-f0850e`. Коммиты: `10c4aaf` (core IndexMap) · `2d7df41` (IPC) · `afac67f` (predicate) · `60b2091` (wire+hint) · `6f9d684` (Cargo.lock) · `110ee31` (**live-fix**: serde_json `preserve_order`). Гейт: `cargo test --workspace` (188+64) · `pnpm test` 1084 · `pnpm build` · bindings no-drift (specta сворачивает `IndexMap`→тот же TS `Record`).

**Live-pass fix (КЛЮЧЕВОЕ — `110ee31`):** порядок не сохранялся вживую, хотя весь гейт был зелёный. Корень: tauri сериализует возвраты команд через `serde_json::to_value`, а `serde_json::Value::Object` — это `BTreeMap` (алфавит) без фичи **`preserve_order`**. Порядок `IndexMap` доживал до файла (прямой `to_writer`, без `Value`-хопа → file-store-тесты зелёные), но пересортировывался по алфавиту на границе IPC. Фикс — включить `serde_json` `preserve_order` в workspace `Cargo.toml` (унификация фич применяет к собственному serde_json внутри tauri). Покрыто регресс-тестом `src-tauri/src/ipc/env.rs::to_value_preserves_variable_insertion_order` (RED→GREEN на границе `to_value`). Побочка: скелеты запросов теперь в proto-порядке (улучшение), тело ответа не затронуто (prost-reflect → `serde_json::Serializer` напрямую). Урок: file-store round-trip НЕ ловит этот класс багов — нужен тест на `to_value`-границу. **Решение по дизайну:** оставлен `IndexMap` (порядок структурный, ноль дрейфа bindings), НЕ поле `order` (денормализация+инвариант) и НЕ `Vec<VarEntry>` (дрейф bindings+переписать фронт); к `Vec<VarEntry>` перейти ТОЛЬКО если понадобятся чисто числовые имена переменных (ECMAScript hoisting) или drag-reorder.
