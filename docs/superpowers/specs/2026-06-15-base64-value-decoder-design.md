...
- **Decode base64** — декодирует **всё** значение под курсором, открывает диалог
  (показывается на строковых значениях, прошедших charset-гейт — см. ниже);
...
функция над **всем** значением, **только по charset**: `^[A-Za-z0-9+/_-]+={0,2}$`
(стандартный **и** URL-safe алфавит), длина ≥ 4 (минимум для одного байта).
**Без верхнего/жёсткого порога длины** — пользователь явно сказал «декодировать
*любую* строку»; «обычно длинные» — наблюдение, не ограничение. Charset-чек лишь
убирает явный шум (строки с пробелами/`:`/прозой). Точные false-positive **не
страшны**: источник истины — бэкенд. Если значение прошло charset, но не
декодируется (длина не кратна 4 и не URL-safe-no-pad) → IPC `Err` → тост «Not
valid base64»; если декодируется в мусор → покажется как `Binary` со сводкой.
То есть «слишком широкий» гейт безопасен (максимум — лишний пункт меню) — намеренный
размен в пользу «декодировать любую строку».
