...
gRPC-поля типа `bytes` сериализуются в JSON как **base64-строки** (канон —
стандартный base64 с паддингом; на декодировании принимаются и URL-safe, и без
паддинга — protobuf JSON mapping, [ProtoJSON spec][protojson]). В ответе такие
значения часто **большие** и непрозрачные: внутри может лежать вложенный JSON,
текст, или бинарь (картинка, gzip, protobuf-блоб). Сейчас пользователь видит
только base64-простыню (у больших значений — 64-символьный preview + бейдж
размера, см. `elide.ts`) и **никак не может заглянуть внутрь**.
...
**Триггер — контекстное меню (ПКМ) по строковому значению в табе Body ответа.**
Пункты:
- **Decode base64** — открывает диалог (показывается только когда значение
  похоже на base64, см. гейт ниже);
- **Copy value** — копирует сырую строку (дублирует существующий
  Ctrl+dblclick-копир, но даёт дискаверабилити);
- **Save decoded to file…** — декодирует и сразу открывает нативный «Save As»
  (без открытия диалога-вьюера).
...
**Гейт «похоже на base64»** (фронт, для показа пункта меню) — чистая функция:
charset `^[A-Za-z0-9+/_-]+={0,2}$` (стандартный **и** URL-safe алфавит), длина
≥ 16 после вырезания пробелов/переносов. Источник истины — всё равно бэкенд: если
строка прошла гейт, но не декодируется, IPC вернёт `Err` → тост «Not valid
base64». Гейт — только чтобы не зашумлять меню на явно-не-base64 значениях.
...
- **`src/features/bodyview/decode.ts`** (новый, чистый): `isLikelyBase64(s)`
  (гейт меню) + `suggestedNameFor(kind, ext)`-хелпер. Юнит-тестируемо.
...
- **Контекстное меню в `BodyView` (mode=response).** В `onMount` регистрируем
  `editor.addAction(...)` для трёх пунктов **без keybinding** (только
  ...
  пункт меню scoped на инстанс). Видимость «Decode» гейтим контекст-ключом
  `hsValueIsB64`, выставляемым в `onContextMenu` по значению под кликом; фолбэк —
  показывать всегда и на `run` при не-base64 кидать тост. Действия (`run`):
  - Decode → `props.onDecode?.(value)` (поднимает значение наверх);
  - Copy value → `copyToClipboard(value)` (как сейчас);
  - Save decoded to file… → `base64_save(value)` + тост по результату.
  Регистрируем **только** в response-mode; диспозим на unmount.
...
| `src/features/bodyview/decode.ts` (+ `.test.ts`) | **новый** — `isLikelyBase64`, хелперы |
...
**Фронт (vitest):**
- `isLikelyBase64`: стандартный/URL-safe → true; короткая (<16) → false;
...
- `BodyView` response-mode: ПКМ-экшены зарегистрированы; `onDecode` зовётся с
  полным значением; request-mode их **не** регистрирует.
