...
Версию приложения (которую зашивает Tauri и сравнивает апдейтер) определяет
`tauri.conf.json > version`; **если это поле убрать, Tauri берёт версию из
`src-tauri/Cargo.toml` (`package.version`)** — проверено по доке Tauri 2
(distribute/versioning). `package.json > version` фронта на версию приложения **не
влияет**. Апдейтер делает semver-сравнение версии собранного приложения с версией в
`latest.json`. **Git-тег версию не задаёт** — её задают файлы; тег лишь триггерит CI.

Процесс: поднять версию → коммит → `git tag vX.Y.Z` → `git push origin vX.Y.Z`
→ CI публикует Release.

**Единый источник правды (детали — в плане):** рекомендуемый вариант — **убрать
`version` из `tauri.conf.json`**, тогда источник правды для версии приложения — один
файл `src-tauri/Cargo.toml`. `package.json` остаётся независимым (фронтовый/npm,
апдейтер его не читает) — синхронить его не обязательно, но можно для порядка.
Альтернатива — оставить `version` в `tauri.conf.json` и держать файлы в синхроне
скриптом/чек-листом.
