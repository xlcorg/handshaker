...
**Статус:** 🎉 DONE (code-complete + влита в `main` fast-forward 2026-06-30;
**живой WebView2-проход ещё НЕ пройден**). План —
`archive/2026-06-30-save-response-to-file.md`.
**Ветка:** `claude/pedantic-colden-c29193`
**Дата:** 2026-06-30

> **Отклонения от спеки (live-фидбек, 2026-06-30):** (1) пункт меню «Save response to
> file…» вынесен в **собственную группу-разделитель** прямо под Collapse/Expand all
> (best-practice: группировать по категории, разделять дивайдерами — NN/g; зеркалит
> VS Code); (2) **иконка Download в шапке убрана** — остались контекстное меню +
> Ctrl/Cmd+S (две точки входа, не три); (3) **имя файла упрощено до
> `response-<localstamp>.json`** (метод/коллекция/сервис в имя НЕ входят).
