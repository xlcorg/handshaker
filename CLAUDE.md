...
`max_decoding_message_size` tonic, не баг в коде). Новый pref **`maxMessageBytes`** (в байтах;
`0` = Unlimited; дефолт **16 MiB**) протянут FE→core зеркалом пути `requestTimeoutMs`:
pref → `actions.ts` → IPC `grpc_invoke_oneshot(max_message_bytes: u32)` → core `invoke_unary`
→ `unary_dynamic` → `tonic::client::Grpc::max_decoding_message_size` +
`max_encoding_message_size` (**один лимит на приём и отправку**). **Сентинел** `0 → usize::MAX`
маппится один раз на границе IPC (`resolve_max_message_size` в команде); ядро о сентинеле не
знает (получает готовый `usize`). **UI** — дискретный слайдер (`src/components/ui/slider.tsx`
на `radix-ui`, новый shadcn-компонент с `thumbLabel→aria-label`) c 12 стопами-степенями
двойки `1…1024 MiB`, крайний правый = **Unlimited**; readout/hint переключаются на «Unlimited»
при стопе `0`; индекс↔байты через `MESSAGE_SIZE_STOPS`/`stopIndexFor`/`formatMessageSize`
(`use-prefs.ts`). **Только invoke-путь** (reflection остаётся на дефолте 4 MiB — отмеченный
follow-up). Строки `NetworkPane` централизованы в `messages.ts` (`settings.network`) по правилу
ui-strings (заодно вынесены существующие inline-строки deadline). Subagent-driven (6 задач TDD,
spec+quality ревью на каждой + финальное ревью = READY TO MERGE). Ребейз на локальный `main`
чист (непересекающиеся пути), ff `045f7ba`. Гейт: `cargo test --workspace` (core 206 ·
src-tauri 71) · vitest 1157 · `pnpm build` (tsc+vite) · bindings no-drift. **Урок (tonic):**
