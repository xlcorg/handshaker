...
Последняя влитая — **Командная палитра — прибита к верху (рост только вниз)**
...
Предыдущая — **Командная палитра — полнотекстовый поиск внутри выбранной коллекции**
(🎉 DONE 2026-06-30, rebase+ff в `main`; план+спека `2026-06-30-scoped-palette-fulltext-search*`
в `archive/`) — в scope-режиме палитры (`Ctrl/Cmd+K|P`, после `Tab` в коллекцию) поиск теперь
матчит **полный текст** запроса — `имя + полный путь сервиса + метод + адрес` — ровно как плоский
режим, а не только имя запроса. До этого, зайдя в коллекцию, нельзя было найти метод по имени его
gRPC-сервиса (в плоском режиме это работало — несогласованность). Правка чистая и точечная: удалён
name-only пост-фильтр в scope-ветке `derivePaletteResults` (`paletteModel.ts`) — scope стал срезом
плоского режима по одной коллекции, переиспользуя `rankRequests` (haystack
`имя + service.method + address`) напрямую. Ранжирование остаётся name-first (бонусы скорера
`fuzzy.ts`: префикс +8, начало слова +3, смежность +5), поэтому service-only «шум» (`sea` →
`GetStatus` через общий сервис) оседает внизу. Подсветка — только по имени
(`matchIndices(query, request.name)`), как в плоском режиме. Бэкенд/IPC/bindings не тронуты (данные
из `cat.tree`). Subagent-driven (1 задача TDD: инверсия `sea`-теста + тест порядка name-first + тест
поиска по фрагменту сервиса `letters`; spec+quality ревью = APPROVED). При вливании — ребейз на
актуальный `main` (`d968f73`, dynamic-builtin-vars): `paletteModel.*` не пересекались → чисто. Гейт:
vitest **1172** · `tsc -b` · `vite build` — зелёные. Остаток — живой WebView2-проход. **Урок:**
scope-поиск должен быть срезом плоского, а не отдельной name-only веткой; см.
...
- **Динамические встроенные подстановки `{{$guid}}` и др.** (🎉 DONE 2026-06-30, rebase+ff в `main`
  `0ba7bfa`; `2026-06-30-dynamic-builtin-vars` / `2026-06-29-dynamic-builtin-vars-design` в
  `archive/`) — 6 встроенных динамических переменных (Postman-стиль с `$`): `$guid`/`$guid7`/
  `$timestamp`/`$unixMs`/`$isoTimestamp`/`$randomInt`, генерируются в момент Send (per-occurrence
  свежесть). Ядро *распознаёт* билтины (поле отчёта `dynamic_vars`, не unresolved); подстановка — в
  `grpc_invoke_oneshot` через инъектируемый трейт `BuiltinGenerator`; пользовательская переменная
  того же имени побеждает. Превью — фиолетовая подсветка `vh-dynamic` + автокомплит; ядро
  детерминировано. Деп: только фича `v4` у `uuid` (ISO-8601 и `$randomInt` — вручную). **Урок
  (Monaco):** `setLanguageConfiguration` без `colorizedBracketPairs:[]` красит парные скобки по
  глубине поверх токена (выключать на уровне языка, не опцией редактора).

- **Раскрытие активного запроса в дереве при открытии** (🎉 DONE 2026-06-30, ff в `main`
  `7c89c6b`; план-дока нет — прямой TDD, чистый фронт) — открытие сохранённого запроса (палитра /
  обзор коллекции / restore при старте) разворачивает предков через `pathToItem` и **скроллит
  строку во вьюпорт** (`scrollIntoView({block:"nearest"})`); зеркалит `editingId`-reveal в
  `CollectionTree.tsx`. Идемпотентно по `activeItemId` (ref-гард — перезагруз каталога не дерётся с
  ручным сворачиванием), transient (не зовёт `onSetExpanded`). Бэкенд/IPC/bindings не тронуты. TDD
  (4 теста). Live-verified в WebView2. Урок — [[project_command_palette_quick_search]].
